...
--  end
  ...
  function energyConsumption(time)
    ...
    a = api.get("/energy/consumption/summary?period="..os.date("%Y-%m-%d",time))
    return a
  end
  ...
  rule([[@{00:07,catch} => 
    local e=energyConsumption(os.time()-30*24*60*60);
  ...
  rule("eConsumption > 1000 => log('Consumption last 30 days exceed 1000 (%s)',eConsumption)")
