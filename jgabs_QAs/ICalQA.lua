...
--%%u4={label='offsetText',text="Time offset: 0"}
--%%u5={label='row1',text=""}
--%%u6={label='row2',text=""}
--%%u7={label='row3',text=""}
...
function QuickApp:main()
  ...
  self:event({type='currentEvents'},function(e)
      ...
      fibaro.postRemote(from,{type='currentEvents',events=entries})
    end)

  self:event({type='todaysEvents'},function(e)
      local  from = e.event._from
      if not from then self:warning("todaysEvents - no reply id") return end
      local now = os.time()
      local entries = {}
      local midnight = fibaro.midnight()
      local endOfDay = midnight+24*3600
      for _,e in ipairs(calendarAlarms) do
        if e.entry.startDate >= midnight and e.entry.endDate <= endOfDay then
          entries[#entries+1]=e.entry
        end
      end
...
--  self:post({type='currentEvents'},10)
  ...
  self:event({type='checkCalendar'},function(e) -- {type='checkCalendar', name=<string>, interval=<seconds>}
      e = e.event
      local events = calendars[e.name].cal.fetchData(e.name)
  ...
  self:event({type='newEntries'}, function(e) -- {type='newEntries', name=name, entries=myCal}
      ...
      for _,e in ipairs(calendarAlarms) do
        if e.name == name then 
          if e.start then fibaro.cancel(e.start) end
          if e.ends then fibaro.cancel(e.ends) end
        else 
          newCal[#newCal+1]=e 
        end
      end
      ...
      for i,entry in ipairs(entries) do
        ...
        calendarAlarms[#calendarAlarms+1] = {calName=entry.calName,start=refS,ends=refE,name=name,uid=entry.uid, entry=entry}
      end
...
-- other variables
  ...
  local function AppendToCal(e, calName)
    local now = os.time()
...
--- v 1.2 whole-day events are defined by date only
  ...
  local function AppendRecuringEntry(recurance, e, calName)
    local i = 1;
    local l_rule;
    local l_finish;
    local l_multiplier;
...
-- parse the entries
  local function CalRead(data,calName)
    local e = {}
...
--   filter to the events of the current month (and year)
          ...
          then
            ...
            AppendRecuringEntry(recurring, e, calName)        
          end -- recurance
        end
      end
    end
  ...
  local function GetICalData(url,calName)
    HC:request(url,{ 
        options = {method = "GET", checkCertificate = false, timeout=20000},
        success = function(response)
          if response.status==301 then
            GetICalData(response.headers.location,calName)
          elseif response.status==200 then
            CalRead(response.data,calName) 
          else
  ...
  -- TESTING=true
  function self.fetchData(calName)
    myCal = {}
    if TESTING then
      local t=os.date("*t")
      ...
      return GetICalData(calUrl,calName)
    end
  end
