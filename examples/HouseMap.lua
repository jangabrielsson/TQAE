-- Documenattion: https://forum.fibaro.com/topic/49113-hc3-quickapps-coding-tips-and-tricks/?do=findComment&comment=244337
...
function ImageMap:rc2i(row,col) return (row-1)*self.columns+col end 
...
function ImageMap:setTile(row,col,tag)
   ...
   self.map[self:rc2i(row,col)] = self.tiles[tag][self:rc2i(row,col)]
end
 
function ImageMap:getTile(row,col)
    assert(row>0 and row <= self.rows and col>0 and col <= self.columns,"map index out of range")
    return self.map[self:rc2i(row,col)]
end
...
print("Image files expected in ",outputFile:match("(.*/)"))

local fo = io.open(outputFile,"w")
assert(fo,"Could not open "..tostring(outputFile))
